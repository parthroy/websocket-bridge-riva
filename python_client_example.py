...
class RivaWebSocketClient:
    ...
    def __init__(
        ...
        # Parse riva_url into host and port components
        if riva_url:
            self.riva_host, self.riva_port = self._parse_riva_url(riva_url)
        else:
            self.riva_host = None
            self.riva_port = None
    ...
    def _parse_riva_url(self, riva_url: str) -> tuple[Optional[str], Optional[int]]:
        """
        Parse Riva URL into host and port components

        Args:
            riva_url: URL in format "host:port" or "host" (defaults to port 50051)
        ...
        try:
            if ":" in riva_url:
                ...
                port = int(port_str)
                logger.info(f"🎯 Parsed Riva URL: {host}:{port}")
                return host, port
            else:
                # No port specified, use default
                logger.info(f"🎯 Using Riva host with default port: {riva_url}:50051")
                return riva_url, 50051
        except ValueError as e:
            logger.error(f"❌ Invalid Riva URL format '{riva_url}': {e}")
            logger.info(
                "💡 Expected format: 'hostname:port' or 'hostname' (defaults to port 50051)"
            )
            return None, None
    ...
    async def connect(self):
        ...
        try:
            ...
            self.websocket = await websockets.connect(
                self.ws_url, ssl=ssl_context, ping_interval=20, ping_timeout=10
            )
    ...
    async def stop_asr_session(self):
        ...
        try:
            ...
            if response_data.get("type") == "end":
                logger.info(
                    f"✅ ASR session ended: {response_data.get('reason', 'Unknown')}"
                )
                self.is_recording = False
...
async def stream_audio_file(
    client: RivaWebSocketClient, audio_file_path: str, chunk_size: int = 4096
):
    """
    ...
    try:
        with wave.open(audio_file_path, "rb") as wav_file:
            ...
            logger.info(
                f"   - Duration: {wav_file.getnframes() / framerate:.2f} seconds"
            )
            ...
            if framerate != client.sample_rate:
                logger.warning(
                    f"⚠️ Audio sample rate ({framerate}) differs from client setting ({client.sample_rate})"
                )
...
async def main():
    ...
    config = {
        ...
        "riva_url": None,  # Set to "hostname:port" for custom Riva server
        # "riva_url": "riva-server.company.com:50052",  # Example
        # "riva_url": "10.0.1.100:50051",              # Example with IP
        # "riva_url": "riva.example.com",              # Example (uses default port 50051)
        "language": "en-US",
        "sample_rate": 16000,
        "encoding": "LINEAR16",
        "verify_ssl": False,
...
if __name__ == "__main__":
    ...
    print("   - Set riva_url for custom Riva server (e.g., 'riva-server.com:50052')")
    print("   - Change language, sample_rate, encoding as needed")
    print()
    print("Examples of riva_url formats:")
    print("   - 'localhost:50051'           # Host with port")
    print("   - 'riva.company.com:50052'    # Domain with custom port")
    print("   - '10.0.1.100:50051'          # IP address with port")
    print("   - 'riva-server.example.com'   # Domain (uses default port 50051)")
    print()
