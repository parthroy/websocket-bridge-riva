...
class SimpleRivaClient:
    ...
    async def connect_and_transcribe(
        ...
        Args:
            ...
            riva_url: str - Complete Riva server URL (e.g., "riva-server.com:50051")
...
async def example():
    ...
    results = await client.connect_and_transcribe(
        audio_bytes, riva_url="custom-riva.example.com:50052", language="es-ES"
    )
    print("Custom Riva results:", results)

    # Transcribe with IP address and port
    results = await client.connect_and_transcribe(
        audio_bytes, riva_url="10.0.1.100:50051", language="hi-IN"
    )
    print("IP address results:", results)
    
    # Transcribe with hostname only (uses default port 50051)
    results = await client.connect_and_transcribe(
        audio_bytes, riva_url="riva-cluster.company.com", language="fr-FR"
    )
    print("Hostname only results:", results)
